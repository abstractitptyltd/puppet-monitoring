node "http.example.com" {
  ...
  # this gets exported to the monitoring server set via the monitoring::params::monitoring_server variable
  # set the monitoring::params::monitoring_service variable to the class that manages the service for that server
  ...
  nrpe::plugin { 'http_daemon':
    sudo => true, # this runs the check with sudo, currently you need to setup the rules manually, that will change as soon as i release my sudo module :)
    plugin => 'extra', # the script lives in the directory defined in the monitoring::nagios_extra_plugins var in hiera
    check_command => "check_daemon httpd --label httpd -H :::/443 -n 5:30 -c sockets=0:150", # the command that gets run. the directory and sudo will get added to the front of the command
  }
